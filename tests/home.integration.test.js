...
test("home page renders the full catalog and supports filtering", (t) => {
  ...
  assert.equal(document.getElementById("homeTotalGames").textContent, "22");
  assert.equal(document.querySelectorAll("#gameGallery .gallery-card").length, 22);
  ...
  assert.equal(document.getElementById("visibleGameCount").textContent, "22");
  ...
  assert.equal(document.getElementById("visibleGameCount").textContent, "12");
});
